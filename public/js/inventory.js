...
function openItemModal(item, isEdit = false) {
  ...
  if (item.barcode) {
    document.getElementById('barcodeValue').textContent = item.barcode;
    
    if (item.imageUrl) {
      document.getElementById('barcodeImage').innerHTML = `<img src="${item.imageUrl}" alt="Barcode" class="img-fluid">`;
    }
  }
...
function setupEventListeners() {
  ...
  document.getElementById('addItemBtn').addEventListener('click', () => {
    ...
    document.getElementById('barcodeImage').innerHTML = '<div class="text-muted">QR code will be generated when item is saved</div>';
    document.getElementById('barcodeValue').textContent = '';
    document.getElementById('barcodeRow').style.display = 'none';
